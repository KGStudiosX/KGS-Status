...
const updateStatusRAM = async() => {
    ...
    nodee.forEach(element => {
        if(element.startsWith("node_memory_MemTotal_bytes")) memTotal = parseFloat(element.replaceAll("node_memory_MemTotal_bytes ", "")); else
        if(element.startsWith("node_memory_MemFree_bytes" )) memFree  = parseFloat(element.replaceAll("node_memory_MemFree_bytes " , "")); else
        if(element.startsWith("node_memory_Buffers_bytes" )) buffers  = parseFloat(element.replaceAll("node_memory_Buffers_bytes " , "")); else
        if(element.startsWith("node_memory_Cached_bytes"  )) cached   = parseFloat(element.replaceAll("node_memory_Cached_bytes "  , ""));
    });
